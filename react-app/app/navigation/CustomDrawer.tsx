...
import { View, Text, Image, TouchableOpacity, Icon } from "react-native";
...
const CustomDrawerContent = ({ navigation, setScreenStyle }) => {
  ...
  useEffect(() => {
    const scale = interpolate(sv, [0, 1], [1, 0.8]);

    const borderRadius = interpolate(sv, [0, 1], [0, 26]);
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.primary }}>
      ...
      >
        <Drawer.Screen name="HomePage">
          {(props) => (
            <RecipesHomePage {...props} drawerAnimationStyle={screenStyle} />
          )}
        </Drawer.Screen>
      </Drawer.Navigator>
    </View>
